#!/usr/bin/env sh
# vclt — bump the software version everywhere it needs to be typed by
...
TARGETS="vclt.json __alpine/APKBUILD __archlinux/PKGBUILD __debian/control __redhat/vclt.spec"

print_usage() {
  cat <<EOF
Usage: $0 <new-version>   (e.g. $0 1.3.2)

Bumps the software version in vclt.json and every __*/ packaging file,
...
echo "Bumping vclt to version $NEWVERSION (release reset to 1 on every distro)"

patch vclt.json \
  "s/\"versionnumber\": *\"[^\"]*\"/\"versionnumber\": \"$NEWVERSION\"/
   s/\"defaultreleasenumber\": *\"[^\"]*\"/\"defaultreleasenumber\": \"1\"/"
verify vclt.json "\"versionnumber\": \"$NEWVERSION\""
verify vclt.json "\"defaultreleasenumber\": \"1\""
...
patch __redhat/vclt.spec \
  "s/^%define _version .*/%define _version $NEWVERSION/
   s/^%define _rel .*/%define _rel 1/"
verify __redhat/vclt.spec "%define _version $NEWVERSION"
verify __redhat/vclt.spec "%define _rel 1"
